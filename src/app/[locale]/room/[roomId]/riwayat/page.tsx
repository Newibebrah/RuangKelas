...
import { HiCheckCircle, HiClock, HiXCircle, HiDocumentText, HiCash, HiExclamationCircle } from "react-icons/hi";
...
export default function RiwayatPage() {
  ...
  const { wallets, bills, periods, payments, loading, error } = useWallets(roomId);
  ...
  const getWalletName = (walletId?: string) => wallets.find((w) => w.id === walletId)?.name || "Wallet tidak ditemukan";
  const getPeriodNum = (periodId: string) => periods.find((p) => p.id === periodId)?.periodNumber || "-";
  const getBillAmount = (walletId?: string) => bills.find((b) => b.walletId === walletId)?.amount || 0;
  const getVerifiedByName = (payment: Payment) => {
    if (!payment.verifiedBy) return undefined;
    return members.find((m) => m.userId === payment.verifiedBy)?.displayName;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {myPayments.length === 0 ? (
        <Card><CardBody>
          <EmptyState icon={<HiCash className="h-12 w-12" />} title="Belum ada riwayat" description="Pembayaran akan muncul di sini" />
        </CardBody></Card>
      ) : (
        <div className="space-y-3">
          {myPayments.map((payment, idx) => (
            <motion.div key={payment.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.03 }}>
              <Card glass>
                <CardBody>
                  <div className="flex items-start gap-3">
                    <div className={`shrink-0 w-10 h-10 rounded-2xl flex items-center justify-center ${
                      payment.status === "paid" ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400"
                      : payment.status === "pending" ? "bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400"
                      : "bg-slate-100 dark:bg-slate-800 text-slate-400"
                    }`}>
                      {payment.status === "paid" ? <HiCheckCircle className="h-5 w-5" /> :
                       payment.status === "pending" ? <HiClock className="h-5 w-5" /> : <HiXCircle className="h-5 w-5" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-semibold text-text-primary">{getWalletName(payment.walletId)}</p>
                        <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${
                          payment.status === "paid" ? "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300"
                          : payment.status === "pending" ? "bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300"
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-bold text-text-primary">{formatRupiah(getBillAmount(payment.walletId))}</p>
                      {(payment.status === "paid" || payment.status === "pending") && (
                        <button onClick={() => setReceiptPayment(payment)}
                          ...
                          <HiDocumentText className="h-3 w-3" />Terima
                        </button>
                      )}
                    </div>
                  </div>
      ...
      {receiptPayment && (
        <ReceiptModal isOpen onClose={() => setReceiptPayment(null)}
          payment={receiptPayment} period={periods.find((p) => p.id === receiptPayment.periodId)}
          amount={getBillAmount(receiptPayment.walletId)}
